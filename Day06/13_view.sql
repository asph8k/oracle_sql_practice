/*
view : 제한적인 자료만 보기 위해 사용하는 가상테이블의 개념이다.
...
*/
SELECT * FROM user_sys_privs; -- 권한 확인용 sql문

-- 단순 뷰
-- 뷰의 컬럼 이름은 함수같은 가상표현식이면 안됨. 별칭을 지어줘야 된다.
...
VIEW에서 사용하는 컬림이 가상열인 경우에도 안됨.
*/
-- name은 가상열 (virtual column)이기 때문에 INSERT 안됨.
-- 별칭으로 지정한 열은 INSERT 불가.
...
-- 조건에 사용되어진 컬럼값을 뷰를 통해서 변경할 수 없게 해주는 키워드.
