-- MARGE : 테이블 병합
/*
UPDATE와 INSERT를 한방에 처리하고 싶을때 사용하는 기능 MERGE

한 테이블에 해당하는 데이터가 있다면 UPDATE를,
없으면 INSERT로 처리해라.
만약 MERGE가 없었다면 해당 데이터의 존재 유무를 일일히 확인하고
IF문을 사용해서 데이터가 있다면 UPDATE, 없다면 else문을 사용하여
INSERT를 하라고 일일히 얘기해야 하는데, MERGE를 통해 쉽체 처리 가능.
*/
...
VALUES(105, '데이비드', '김', 'DAVIDKIM', '22/04/27', 'IT_PROG');
...
-- 기존의 데이터는 email, phone_number, salary, commission_pct, manager_id, dept_id
-- 는 업데이트 하도록 처리.
-- 새로 유입된 데이터는 그대로 추가.
